{ pkgs, lib, ... }:
let
  inherit (lib) listToAttrs attrsToList mkForce optionalAttrs length;
  inherit (builtins) hasAttr filter elem isNull;
in
{
  ...
  mkUsers = userids: users: (lib.listToAttrs (
    map
      (x: {
        name = x.name;
        value = { uid = mkForce userids.${x.name}.uid; }
  ...
  mkGroups = userids: groups: (lib.listToAttrs (map
    (x: {
      name = x.name;
