...
#
{
  ...
  # disko.devices.disk.sdx99_FIXME.content.partitions.boot.content.mountpoint
    ...
    content = {
      ...
      partitions = {
        ...
        MBR = {
          ...
          size = config.my.fs.partitions.legacyBIOS.size;
        ...
        EFI = {
          ...
          type = "EF00";
          size = config.my.fs.partitions.EFI.size;
          content = {
            type = "filesystem";
            format = "vfat";
            mountpoint = "/boot/efis/${drive}-part${toString config.my.fs.partitions.EFI.id}";
            mountOptions = ["umask=0077" "x-systemd.idle-timeout=1min" "x-systemd.automount" "noauto"];
          };
        };
        boot = {
          name = "boot${toString idx}";
          size = config.my.fs.partitions.boot.size;
          alignment = 8;
          content = {
            mountpoint = "/boot";
            type = "zfs";
        ...
        root = {
          ...
          size = config.my.fs.partitions.root.size;
          alignment = 8;
          content = {
            type = "zfs";
            pool = "zroot";
        ...
        swap = {
          ...
          size = config.my.fs.partitions.swap.size;
          alignment = 8;
          content.type = "none";
        };
        spare = {
          name = "spare-${toString idx}";
          size = config.my.fs.partitions.spare.size ? "100%";
          alignment = 8;
          content.type = "none";
        };
      };
            ...
            vdev = [
              {
                mode = "mirror";
                members =
                  map (d: "/dev/disk/by-id/${d}_part${toString config.my.fs.partitions.boot.id}") config.my.fs.mirrorDrives;
              }
            ];
          };
        };
            ...
            vdev = [
              {
                mode = "mirror";
                members =
                  map (d: "/dev/disk/by-id/${d}_part${toString config.my.fs.partitions.root.id}") config.my.fs.mirrorDrives;
              }
            ];
            # special = {
            #   members = ["z"];
